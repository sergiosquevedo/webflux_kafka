...
public class ProductServiceImpl implements ProductService {
    ...
    public Mono<Product> getProductByCode(int code) {
        return productsRepository.findById(String.valueOf(code))
                .map(this::mapProductEntityIntoProduct);
    }
    ...
    public Mono<Product> saveProduct(Product product) {
        return isProductExist(product.getCode())
                ...
                .map(p -> product)
                .map(p -> new ProductEntity(String.valueOf(p.getCode()), p.getName(), p.getCategory(), p.getPrice(), p.getStock()))
    ...
    public Mono<Void> deleteProduct(int code) {
        return productsRepository.findById(String.valueOf(code))
                .doOnNext(p -> logger.info("Deleting product with code " + p.getId()))
                .flatMap(p -> productsRepository.deleteById(String.valueOf(code)))
                .then();
    ...
    private Product mapProductEntityIntoProduct(ProductEntity p) {
        return new Product(Integer.parseInt(p.getId()), p.getName(), p.getCategory(), p.getPrice(), p.getStock());
    }
}
